...
from models.bricks.misc import Conv2dNormActivation, WTConv2dNormActivation
...
class RepVggPluXBlock(nn.Module):
    def __init__(
        ...
        # CHANGE HERE
        # self.conv1 = WTConv2dNormActivation(
        ...
        # self.conv2 = WTConv2dNormActivation(
...
class RepVGGPluXNetwork(nn.Module):
    def __init__(
        ...
        for idx in range(1, len(out_channels_list)):
            # CHANGE HERE
            # lateral_conv_module = WTConv2dNormActivation(
        ...
        for idx in range(len(in_channels_list) - 1):
            # change from Conv2dNormActivation to WTConv2dActivation
            # downsample_block_module = WTConv2dNormActivation(
    ...
    def forward(self, x: OrderedDict):
        ...
        for idx in range(len(inner_outs) - 1):  # inner_out: [2,256,96,96]
            feat_low = results[-1]   # [2,256,96,96]
            feat_high = inner_outs[idx + 1]   # [2,256,48,48]
