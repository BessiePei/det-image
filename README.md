...
```
python tools/train.py projects/configs/co_deformable_detr/co_deformable_detr_r50_1x_coco.py --work-dir workdir
```
if appear ModuleNotFoundError:from projects import *
run following
```
...
```cmd
python tools./test.py workdir\co_deformable_detr_r50_1x_coco.py workdir\latest.pth --work-dir workdir/metrics --eval bbox
```
for else options change the function `evaluate` in `mmdet/datasets/coco.py`.
