...
def train_one_epoch_acc(
        model: object, optimizer: object, data_loader: object, epoch: object, print_freq: object = 50, max_grad_norm: object = -1, accelerator: object = None
) -> object:
...
def evaluate_acc(model, data_loader, epoch, accelerator=None):
    ...
    for images, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        for target, output in zip(targets, outputs):
            det_res[target['path']] = get_bbox(output)
    ...
    save_path = r'D:\SFDSA\rtdetrv2_pytorch\output\rtdetrv2_r18vd_120e_coco_lR\salience_result.pkl'
    import joblib
    joblib.dump(det_res, save_path)
    print('success')
    exit(0)
...
def get_bbox(output, threshold=0.50):
    res = []
